...
Users can view thier shopping cart by clicking on the cart button that appears in the header. The shopping cart is rendered in a Modal. The modal has a backdrop which blocks interaction with the rest of the page. Here I used React Portals to render the modal. When a new item is added to the shopping cart I checked that if this item is already a part of the cart or not. If the item is in the cart already I only update the amount and the price for that item, if not I added it as a new item in that array.
